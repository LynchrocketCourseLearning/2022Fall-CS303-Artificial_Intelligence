...
class ProblemSolvingAgent:
    ...
    def DFS(self, obstacles, start_pos, goal_pos):
        ...
        while len(stack) > 0:
            ...
            neighbours = self.neighbours_of(obstacles, var)
    ...
    def UCS(self, obstacles, start_pos, goal_pos):
        ...
        while len(frontier) > 0:
            ...
            neighbours = self.neighbours_of(obstacles, state[1])
            for nei in neighbours:
                if nei[0] not in visited:
                    heapq.heappush(frontier, (nei[1], nei[0]))
                    visited.append(nei[0])
                    parents[nei[0]] = state[1]
    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        frontier = []
        parents = {start_pos: None}
        heapq.heappush(frontier, (self.euclidean_distance(start_pos, goal_pos), start_pos))
        visited.append(start_pos)
        while len(frontier) > 0:
            state = heapq.heappop(frontier)
            if state[1] == goal_pos:
            ...
            for nei in neighbours:
                if nei[0] not in visited:
                    dis = self.euclidean_distance(nei[0], goal_pos)
                    heapq.heappush(frontier, (nei[1]+dis, nei[0]))
                    visited.append(nei[0])
                    parents[nei[0]] = state[1]
