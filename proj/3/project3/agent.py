...
class Agent:
    ...
    def get_action(self,
        ...
        ctps_inter_list = list(map(lambda x: x.requires_grad_(True), ctps_inter_list))
        
        real_score_list = [evaluate(compute_traj(x), target_pos, class_scores[target_cls], RADIUS) for x in ctps_inter_list]
        print(max(real_score_list))
        
        # my_lr = 0.260
        # optimizer_list = [torch.optim.Adam([x], lr=my_lr, maximize=True) for x in ctps_inter_list]
        
        # for epoch in range(14):
        #     # optimizer_list = list(map(lambda x: x.zero_grad(), optimizer_list))
        #     gra_score_list = [self.evaluate_modified(compute_traj(x), target_pos, class_scores[target_cls], RADIUS) for x in ctps_inter_list]
        #     gra_score_list = list(map(lambda x: x.backward(), gra_score_list))
        #     # optimizer_list = list(map(lambda x: x.step(), optimizer_list))
        #     optimizer_list = list(map(optimizer_step, optimizer_list))
        #     ed = time.time()
        #     if ed - st >= 0.25:
        #         break
        real_score_list = [evaluate(compute_traj(x), target_pos, class_scores[target_cls], RADIUS) for x in ctps_inter_list]
