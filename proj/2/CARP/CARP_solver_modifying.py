...
def main(args: argparse.ArgumentParser):
    ...
    random.seed(SEED)
    # routes, total_cost = init_solution(demands)
    # print(get_res(routes, total_cost))
    routes, total_cost = simulated_annealing(demands)
    print(get_res(routes, total_cost))
...
def flip(routes: list, total_cost: float):
    ...
    return new_routes, new_total_cost

def swap(routes: list, total_cost: float):
    '''
    :param
        routes: list of the edge list, i.e., list[list[tuple]],
                where edge is in tuple(BEGIN_NODE, END_NODE, COST, DEMAND)
    ...
    for edge1_idx in range(len(route1)-1):
        for edge2_idx in range(len(route2)-1):
            ...
            if new_total_cost + diff1 + diff2 < new_total_cost:
                ...
                new_route1 = route1[:edge1_idx]+[edge2]+route1[edge1_idx:]
                new_route2 = route1[:edge2_idx]+[edge1]+route2[edge2_idx:]
...
def new_solution(routes: list, total_cost: float):
    ...
    '''
    new_solutions = [flip(routes, total_cost), swap(routes, total_cost)]
